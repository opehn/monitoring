...
static char	*make_log_filename(int aid)
{
	time_t		cur_time;
	...
	sprintf(file_name, "./log/agent_log/%d_%d-%02d-%02d", aid, year, month, day);
	return (file_name);
}
...
static int daemonize(int aid)
{
	...
	filename = make_log_filename(aid);
...
//	logfd = daemonize(aid);
	...
	char *filename;
	filename = make_log_filename(aid);
	if (0 > (logfd = open(filename, O_RDWR | O_APPEND | O_CREAT | O_NOCTTY, S_IRWXU)))
	{
